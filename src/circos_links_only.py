...
CIRCOS_TEMPLATE = """
karyotype = circos/karyotype.human.h37d5.txt
...
class Region:
    def __init__(self, chrom, pos1, pos2, value, color = "black", z = 1):
        ...
        self.z = z
    def __str__(self):
        attr = ["color=%s,fill_color=%s,z=%d" % (self.color, self.color, self.z)]
...
def plot(ofn, links, histogram, text, chroms = None):
    o = open("circos.conf", "w")
    if chroms:
        prelim = "chromosomes_display_default = no\nchromosomes = %s\n" % (";".join(chroms))
    else:
        prelim = ""
    params = {"prelim" : prelim, "linkfn" : "links.txt", "histogramfn" : "histogram.txt", "imagefn" : ofn, "imagedir" : "."}
    ...
    subprocess.call("%s -conf circos.conf -outputdir %s -outputfile %s" % (CIRCOS, os.path.dirname(ofn), os.path.basename(ofn)), shell = True, stdout = subprocess.PIPE)
